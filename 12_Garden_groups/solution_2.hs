...
area = S.size
                                ...
                                -- inner  border
boundary :: Region -> Set (Coord, ([Dir], [Dir]))
boundary r = S.filter (not . null . snd . snd) $ S.map withConnection r
  where withConnection c = (c, L.partition (\d -> go d c `S.member` r) dirs)
